...
Cascade Stress에서 부모 상태 변경(COMPLETED↔TODO) 시 `findAllForTreeByUserId`로 유저의 **전체 트리**를 메모리에 올린 뒤 BFS로 자손을 순회하는 구조가 병목이었습니다.

Recursive CTE로 대상 노드의 자손 ID만 조회하도록 교체하자 p95 8.77s → 7.69s, max 20.39s → 12.78s, 처리량 14.6 → 17.5 req/s로 개선됐습니다.
