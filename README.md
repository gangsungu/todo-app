...
K6로 세 가지 시나리오를 측정했습니다 (로컬, WSL2 + Docker MySQL 8.0).

| 시나리오 | VU | 에러율 | p95 | 비고 |
|----------|----|--------|-----|------|
| Smoke | 5 | 0% | 24ms | 임계값(500ms) 통과 |
| Load | 50 | 0% | 4.39s | 임계값(1s) 초과 — POST/PATCH write 경합 |
| Cascade Stress | 100 | 0% | 7.69s | cascade 쿼리 병목 측정 목적 |

**병목 원인 및 개선**

Cascade Stress에서 부모 상태 변경(COMPLETED↔TODO) 시, 변경 대상 노드와 무관한 유저의 전체 트리 데이터를 매번 DB에서 조회하여 메모리에 올린 후, 애플리케이션 레벨에서 BFS로 자식 노드를 탐색하는 구조가 불필요한 I/O와 메모리 누수를 유발하는 병목이었습니다.

이것을 MySQL 8.0의 Recursive CTE 구조로 변경하여 풀 스캔없이 하위 자식 ID만 빠르게 조회하도록 수정하였습니다. 결과적으로 p95 응답속도는 약 12%, 최대 대기 시간은 37% 개선(20.39s -> 12.78s)되고 초당 처리량도 20% 상승하는 성과를 얻었습니다.

| 시나리오 | VU | 에러율 | p95 (개선 전) | p95 (개선 후) | 비고 | 
|:--- |:---: |:---: |:---: |:---: |:--- | 
| Smoke | 5 | 0% | - | 24ms | 임계값(500ms) 통과 | 
| Load | 50 | 0% | - | 4.39s | 임계값(1s) 초과 — POST/PATCH write 경합 | 
| Cascade Stress | 100 | 0% | 8.77s | 7.69s | Recursive CTE 도입으로 자손 노드 조회 최적화 |

자세한 수치는 [`k6/README.md`](k6/README.md)를 참고하세요.
